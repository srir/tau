...
def grab_twitter_data(lat, longitude, rad):
    data = []
    rurl = "https://search.twitter.com/search.json?q=since:2012-05-15&geocode={0},{1},{2}km&rpp=100&page=1".format(lat,longitude, rad)
    print "OMGURL========\n\n\n {0}\n\n\ =========".format(rurl)
    #for i in range(1,3):
    r = requests.get(rurl)
    return json.loads(r.text)
...
def get_by_loc(coord, radius):
    ...
    radius = str(111.12*radius)
    singlyData = grab_singly_data(latitude, longitude, radius)
    twitterData = grab_twitter_data(latitude, longitude, radius)
    for item in singlyData:
        textlist.append("Singly:"+item['data']['text'])
    for item in twitterData['results']:
        textlist.append("Twitter:"+item['text'])
    return textlist
...
def calc_squares(start_lat, start_long, chunks, size):
    ...
    for i in range(-2, 1):
        for j in range(-2, 1):
            ...
            print 'average: ' + str(square_mean.mean)
